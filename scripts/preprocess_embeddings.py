...
def generate_embeddings(
    cancer_type, data_dir, use_pathway=True, prompt_version='v5', dataset_name=None
):
    """
    生成病人 embedding 的主函数
    ...
    Args:
        ...
        prompt_version: frozen prompt implementation (v1, v2, v4, or v5)
    """
    print(f"="*60)
    ...
    # Resolve the physical cohort independently from its biological cancer
    # label so external datasets do not accidentally acquire a TCGA prefix.
    cohort_name = dataset_name or (
        cancer_type if cancer_type.startswith('TCGA-') else f"TCGA-{cancer_type}"
    )
    expr_candidates = [
        os.path.join(data_dir, f"{cohort_name}.star_fpkm.tsv.gz"),
        os.path.join(data_dir, f"{cohort_name}.expression.tsv.gz"),
    ]
    expr_file = next((p for p in expr_candidates if os.path.exists(p)), expr_candidates[0])
    surv_file = os.path.join(data_dir, f"{cohort_name}.survival.tsv.gz")
    output_path = os.path.join(data_dir, f"{cohort_name}_embeddings_{prompt_version}.pt")
    report_path = os.path.join(data_dir, f"{cohort_name}_reports_{prompt_version}.txt")
    pathway_scores_path = os.path.join(data_dir, f"{cohort_name}_pathway_scores.csv")
    ...
    for i in tqdm(range(len(expr_df)), desc="Processing patients"):
        ...
        if use_pathway and pathway_scores is not None:
            ...
            if prompt_version == 'v5':
                prompt = generate_pathway_prompt_v5(
                    patient_id, pathway_scores.iloc[i], expr_df_symbols.iloc[i], cancer_type
                )
            elif prompt_version == 'v4':
                prompt = generate_pathway_prompt_v4(
                    patient_id, pathway_scores.iloc[i], expr_df_symbols.iloc[i], cancer_type
                )
        ...
        # LLM Reasoning
        if prompt_version == 'v5':
            report = generate_report_v5(client, prompt)
        elif prompt_version == 'v4':
            report = generate_report_v4(client, prompt)
        else:
...
def generate_external_embeddings(dataset_name, data_dir, cancer_type=None):
    ...
    # Use the same frozen workflow while preserving the external cohort name.
    generate_embeddings(
        cancer_type=cancer_type or dataset_name,
        data_dir=data_dir,
        use_pathway=True,
        prompt_version='v5',
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--prompt_version', type=str, default='v5', choices=['v1', 'v2', 'v4', 'v5']
    )
