...
class UnifiedDataLoader:
    ...
    def load_dataset(
        ...
        if genes is not None:
            if allow_missing_features:
                # Preserve the frozen development feature order. Missing target
                # features are inserted and later set to the development mean
                # (zero after standardisation), rather than changing the model
                # input dimension or selecting features from the target cohort.
                use_genes = list(genes)
                missing_feature_mask = np.array(
                    [g not in expr_df.index for g in use_genes], dtype=bool
                )
            else:
            ...
            if not allow_missing_features and len(available_genes) < len(genes) * 0.5:
                # 尝试去版本号匹配
        ...
        for sample in common_samples:
            if sample in sample_to_idx:
                ...
                # 提取表达数据
                expr_vec = expr_df.reindex(use_genes)[sample].fillna(0).values
        ...
        if missing_feature_mask is not None and missing_feature_mask.any():
            gene_normalized[:, missing_feature_mask] = 0.0
            print(
                f"    Missing frozen features set to development mean: "
                f"{missing_feature_mask.sum()}/{len(missing_feature_mask)}"
            )
    ...
    def load_paired_datasets(
        ...
        # 2. Load expression data. Feature selection is performed from the
        # development cohort only; target-platform availability must not alter
        ...
        print("\n[Step 1] Freezing development-only feature manifest...")
        train_genes = list(
            train_expr.var(axis=1).sort_values(ascending=False).head(self.n_genes).index
        )

        test_exact = set(test_expr.index)
        test_by_stripped = {
            strip_ensembl_version(g): g for g in test_expr.index
        }
        symbol_to_ensembl = {
            symbol: ensembl for ensembl, symbol in (gene_mapping or {}).items()
        }

        test_genes = []
        for position, train_gene in enumerate(train_genes):
            mapped = None
            if train_config.gene_id_type == test_config.gene_id_type:
                if train_gene in test_exact:
                    mapped = train_gene
                else:
                    mapped = test_by_stripped.get(strip_ensembl_version(train_gene))
            elif train_config.gene_id_type == 'ensembl' and test_config.gene_id_type == 'symbol':
                symbol = (gene_mapping or {}).get(strip_ensembl_version(train_gene))
                if symbol in test_exact:
                    mapped = symbol
            elif train_config.gene_id_type == 'symbol' and test_config.gene_id_type == 'ensembl':
                ensembl = symbol_to_ensembl.get(train_gene)
                if ensembl is not None:
                    mapped = test_by_stripped.get(strip_ensembl_version(ensembl))

            # A unique sentinel preserves column order and is converted to the
            # development mean after standardisation by load_dataset.
            test_genes.append(mapped or f"__MISSING_FROZEN_FEATURE_{position}__")

        missing_count = sum(g.startswith('__MISSING_FROZEN_FEATURE_') for g in test_genes)
        print(
            f"    Frozen features: {len(train_genes)}; "
            f"unavailable in target: {missing_count}"
        )
        ...
        info = {
            ...
            'n_missing_external_features': missing_count,
